 ...
 public:
  ObservationData(std::string observation_data_str) {
    ...
    if (str_stream.good()) {
      ...
      for (int i=0; i<3; ++i) {                    
        std::getline(str_stream, data_str, ','); 
        landmark_pos_(i) = std::stod(data_str);
      }
    }
  }
  ...
  Eigen::Vector3d GetLandmarkPosition(){
    return landmark_pos_;
  }
...
class ExpLandmarkOptSLAM {
 ...
 public:
  ...
  bool ReadObservationData(std::string observation_file_path) {
    ...
    while (std::getline(input_file, observation_data_str)) {
      ...
      obs_data_vec.push_back(observation_data);
      ...
      if (landmark_id >= landmark_parameter_.size()) {
        landmark_parameter_.push_back(new LandmarkParameterBlock(observation_data.GetLandmarkPosition(), landmark_id));
      }
    }
  ...
  bool SolveOptimizationProblem() {
    ...
    for (size_t i_obs = 0; i_obs < obs_data_vec.size(); i_obs++){
        ...
        // optimization_problem_.AddResidualBlock(cost_function2,
        //                                         NULL,
        //                                         position_parameter_.at(i_imu_lastframe)->parameters(),
        //                                         velocity_parameter_.at(i_imu_lastframe)->parameters(),
        //                                         rotation_parameter_.at(i_imu_lastframe)->parameters(),
 ...
 private:
  ...
  ceres::Solver::Summary optimization_summary_;
      ...
      // storage of IMU and obs data
    std::vector<IMUData> imu_data_vec;
    std::vector<ObservationData> obs_data_vec;
};
...
int main(int argc, char **argv) {
  ...
  std::string euroc_dataset_path = (std::string)argv[1]+ "/";
